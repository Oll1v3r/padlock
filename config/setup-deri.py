...
def fin():
    ...
    system('%s' % (tamanhoPosicaoF))
    system('%s' % (invisivel))
    print("\r%sCopiando Sign para /usr/bin/Sign          %s[%sno%s]" % (darkgreen, blue, yellow, blue))
    sleep(0.5)
...
def processa():
    ...
    #mod = 'sed -i "s|from colors import dict_colors|from Sign.colors import dict_colors|g" Sign.py'
    comm = 'rm Sign.py'
