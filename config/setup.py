...
def fin():
    ...
    system(tamanhoPosicaoF)
    system(invisivel)
    print("\r%sCopiando log-termux para /sdcard/log-termux.txt          %s[%sno%s]" % (darkgreen, blue, yellow, blue))
    sleep(1)

    system(tamanhoPosicaoF)
    system(invisivel)
    print("\r%sCopiando log-termux para /sdcard/log-termux.txt          %s[%sok%s]" % (darkgreen, blue, yellow, blue))
    sleep(1)
    
    tamanhoPosicaoF = 'tput cup 27'
    system('%s' % (tamanhoPosicaoF))
...
def processa():
    ...
    if (path.exists('Sign.py') == True):
        ...
        def registro():
            ...
            system('cp log-termux.txt /sdcard/')
            system('rm log-termux.txt')
